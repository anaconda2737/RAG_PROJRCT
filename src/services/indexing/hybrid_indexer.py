...
class HybridIndexingService:
    ...
    async def index_paper(self,paper_data:Dict)-> Dict[str,int]:
        ...
        try:
            ...
            if not chunks:
                ...
                return {"chunks_created":0,"chunks_indexed":0,"embedding_generated":0,"errors":0}
            ...
            if len(embeddings)!= len(chunks):
                ...
                return {"chunks_created":len(chunks),"chunks_indexed":0,"embedding_generated":len(embeddings),"errors":1}
            ...
            logger.info(f"Indexed Paper {arxiv_id}: {results['success']} chunks successfully ,{results['failed']} failed")

            return {
                    "chunks_created":len(chunks),
                    "chunks_indexed":results['success'],
                    "embedding_generated": len(embeddings),
    ...
    async def index_paper_batch(self,papers:List[Dict],replace_existing:bool=False)->Dict[str,int]:
        total_stats = {
            "papers_processed":0,
            "total_chunks_created":0,
            "total_chunk_indexed":0,
            "total_embedding_generated":0,
            "total_errors":0
        }    
        ...
        for paper in papers:
            ...
            if replace_existing and arxiv_id:
                self.opensearch_client.delete_paper_chunk(arxiv_id)

            stats = await self.index_paper(paper) 

            total_stats["papers_processed"]+=1
            total_stats["total_chunks_created"]+=stats["chunks_created"]
            total_stats["total_chunk_indexed"]+=stats["chunks_indexed"]
            total_stats["total_embedding_generated"]+=stats["embedding_generated"]
            total_stats["total_errors"]+=stats["errors"]

        logger.info(
            f"Batch indexing complete: {total_stats['papers_processed']} papers"
            f"{total_stats["total_chunk_indexed"]} chunks indexed"
        )    

        return total_stats

    async def reindex_paper(self,arxiv_id:str,paper_data:Dict)->Dict[str,int]:
        deleted = self.opensearch_client.delete_paper_chunk(arxiv_id)
        if deleted:
            logger.info(f"Deleted existing chunks for for paper {arxiv_id}")
