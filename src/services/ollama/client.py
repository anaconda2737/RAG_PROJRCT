...
from src.services.ollama.prompts import RAGPrompBuilder,ResponseParser
...
class OllamaClient:
    def __init__(self,settings:Settings):
        ...
        self.prompt_builder = RAGPrompBuilder()
    ...
    async def health_check(self)->Dict[str,Any]:
        try:
            ...
            raise OllamaException(f"Ollama health checked failed: {str(e)}")
    ...
    async def generate(self,model:str,prompt:str,stream:bool=False,**kwargs)-> Optional[Dict[str,Any]]:
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                data = {"model":model,"prompt":prompt,"strem":stream,**kwargs}
                ...
                if response.status_code == 200:
                    ...
                    if "prompts_eval_count" in result:
                        usage_metadata["prompts_token"]= result.get("prompt_eval_count",0)
                    if "eval_count" in result:
                        usage_metadata["completion_tokens"] = result.get("eval_count",0)

                    if usage_metadata:
                        usage_metadata["total_token"] = (
                            usage_metadata.get("prompt_token",0)+
                            usage_metadata.get("completion_token",0)
    ...
    async def  generate_stream(self,model:str,prompt:str,**kwargs):
        try:
            ...
            raise OllamaConnectionError(f"Cannot connect to ollama servie: {e}")
        except httpx.TimeoutException as e:
            raise OllamaTimeOutError(f"Ollama serivice timeout: {e}")
        except OllamaException:
            raise
        except Exception as e:
    ...
    async def list_models(self)->List[Dict[str,Any]]:
        try:
            ...
            raise OllamaConnectionError(f"Can not connect to Ollama service:{e}")
        except httpx.TimeoutException as e:
            raise OllamaTimeOutError(f"ollama service timeout:{e}")
        except OllamaException:
            raise
        except Exception as e:
            raise OllamaException(f"Error listings models:{e}")

    async def generate(self,model:str,prompt:str,stream:bool = False,**kwargs)->Optional[Dict[str,Any]]:
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                data = {"model":model,"prompt":prompt,"stream":stream,**kwargs}
                logger.info(f"Sendint request to Ollama model")
                response = await client.psot(f"{self.base_url}/api/genrstor", json = data)
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(f"Usage metadat:{usage_metadata}")
                    return result
                else:
                    raise OllamaException(f"Generation failed:{response.status_code}") 
            ...
            raise OllamaException(f"Error generaating with Ollama:{e}")

    async def generate_rag_answer(
            self,
            query:str,
            chunks:List[Dict[str,Any]],
            model:str =  "I will choose later",
        ...
        try:
            if use_structured_output:
                ...
                prompt = self.prompt_builder.create_rag_prompts(query,chunks)
            ...
            if response  and "response" in response:
                ...
                if use_structured_output:
                    ...
                    citations =  list(set(chunk.get("arxiv_id") for chuk in chunks if chunk.get("arxiv_id")))
    ...
    async def generate_rag_answer_stream(
                    ...
                    query:str,
                    chunks:List[Dict[str,Any]],
                    model:str = "I will provide this later"
