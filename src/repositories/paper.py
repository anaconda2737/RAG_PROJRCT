...
class PaperRepository:
    ...
    def get_processed_paper(self, limit:int = 100, offset:int = 0)->  List[Paper]:
        stmt =  select(Paper).where(Paper.processed == True).order_by(Paper.published_date.desc()).limit(limit).offset(offset)
        return list(self.session.scalars(stmt))

    def get_paper_with_raw_text(self, limit:int = 100, offset:int = 0)-> List[Paper]:
    ...
    def get_processing_stats(self)-> dict:
        ...
        return {
            ...
            "text_extractiion_rate": (papers_with_text/processed_papers * 100) if processed_papers > 0 else 0
    ...
    def upsert(self,paper_create:PaperCreate)-> Paper:
        existing_paper =  self.get_all_by_arxiv_id(paper_create.arxiv_id)
        if existing_paper:
